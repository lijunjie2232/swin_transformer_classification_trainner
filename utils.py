...
# for dir return True if dir existed or not existed but created
# for file return False if file exists no matter if dir is created
# could create dir but could not touch file
def pathChecker(path, createDir=True):
    pre,suf = os.path.splitext(path)
    if suf == '': # path is a dir name
        ...
        if os.path.exists(path):
            return True
        pathChecker(os.path.dirname(path), createDir=True)
    return False
